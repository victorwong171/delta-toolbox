...
func (sp *SequentialNCMParser) Parse(r io.Reader) (ParsedNCM, error) {
	// 1. 读取 NCM 头部标志位魔数 (8 字节 magic + 2 字节 gap 填充)，使用栈分配缓冲区消除堆逃逸
	var header [10]byte
	...
	if len(modifyData) > 0 {
		...
		// 剔除 "163 key(Don't modify):" 前缀 (22字节) 后原位 (in-place) 进行 Base64 解码消除额外堆分配
	...
	// 4. 跳过 9 字节的 CRC/Gap 空白校验块，使用栈分配缓冲区消除堆逃逸
	var gap [9]byte
