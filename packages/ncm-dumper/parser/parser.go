...
func (sp *SequentialNCMParser) Parse(r io.Reader) (ParsedNCM, error) {
	...
	deKeyData, err := decryptAes128Ecb(aesCoreBlock, keyData) // 使用 AES-128-ECB 解密
	if err != nil {
...
func decryptAes128Ecb(block cipher.Block, data []byte) ([]byte, error) {
	...
	for i := 0; i <= len(data)-bs; i += bs {
		// Perform in-place decryption directly on 'data' slice to eliminate dynamic output buffer allocations
