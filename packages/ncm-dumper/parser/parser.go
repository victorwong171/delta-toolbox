...
func (sp *SequentialNCMParser) Parse(r io.Reader) (ParsedNCM, error) {
	...
	deKeyData := decryptAes128Ecb(aesCoreBlock, keyData) // 使用 AES-128-ECB 解密
	if len(deKeyData) < 17 {
...
// readLenAndData reads 4 bytes of length information directly into a stack-allocated
// buffer and decodes it without reflection or heap allocations.
