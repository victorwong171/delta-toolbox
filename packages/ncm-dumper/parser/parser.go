...
// Read 执行流式解密，集成了编译器边界检查消除 (BCE) 以及循环展开 (loop unrolling) 等指令级微优化以极大提升解密吞吐率
func (dr *DecryptReader) Read(p []byte) (n int, err error) {
	...
	if n > 0 {
		...
		i := 0
		// Unroll the loop by 8 to reduce loop control overhead and enable instruction-level parallelism (ILP)
		for ; i <= n-8; i += 8 {
			offset++
			p[i] ^= lookup[offset]
			offset++
