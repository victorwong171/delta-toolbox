...
func (dr *DecryptReader) Read(p []byte) (n int, err error) {
	...
	if n > 0 {
		...
		// Unroll the loop by 8 to reduce loop overhead and enable instruction-level parallelism (ILP)
		i := 0
		for ; i <= n-8; i += 8 {
			_ = p[i+7] // BCE assertion to eliminate bounds checks for all 8 accesses
			offset++
			p[i] ^= lookup[offset]
			offset++
			p[i+1] ^= lookup[offset]
		...
		// Remainder loop for elements that don't fit in groups of 8
		if i < n {
			_ = p[n-1] // BCE assertion for remainder loop
			for ; i < n; i++ {
				offset++
				p[i] ^= lookup[offset]
			}
