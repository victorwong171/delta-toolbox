...
## 2026-07-18 - Go Binary Parsing, AES Block Reuse, and Fixed-Array Stack Allocation
**Learning:** Standard library `binary.Read` uses reflection to decode data, leading to high CPU overhead and heap escape allocations. Replacing it with reading into a stack-allocated byte buffer followed by `binary.LittleEndian.Uint32` reduces overhead and prevents allocations. Furthermore, compiling constant AES keys once during `init()` into stateless `cipher.Block` variables avoids repeating `aes.NewCipher` heap allocations on every execution. Finally, returning fixed-size arrays (e.g. `[256]byte`) instead of slices from key generator functions completely prevents heap escape.
**Action:** Always replace reflection-based `binary.Read` with stack-allocated buffers and manual byte conversions, cache stateless cipher blocks at package-level, and return fixed-size arrays instead of slices for short buffers.
