...
## 2026-08-12 - In-Place Block Cipher Decryption and Reflection-Free Binary Decoding
**Learning:** Go's standard `cipher.Block.Decrypt(dst, src []byte)` interface supports in-place operations where `dst` and `src` share the exact same slice buffer. Leveraging in-place decryption completely eliminates the allocations of intermediary decrypted buffers. Additionally, replacing reflection-driven `binary.Read` with a stack-allocated byte array (`[4]byte`) and direct standard library byte manipulation (e.g., `binary.LittleEndian.Uint32`) completely removes reflection overhead and dynamic heap allocations, reducing the overall parser allocation footprint by over 9.1% (saving 5 allocations per operation).
**Action:** When performing cryptographic decryption or binary-level length-parsing on transient streams, always decrypt standard block ciphers in-place, and always decode integers using stack arrays paired with direct `binary.LittleEndian` conversions rather than reflection-based `binary.Read`.
